...
**Status**: In Progress — opt-in proactive-post gate landed (`ambient` package; default-OFF two-layer; fail-closed; injected judge/budget) AND wired into the inbound path (`apps/api/ambient-tap.ts` `tapAmbient`): the un-addressed branch now evaluates the gate, audits every decision, and on approval enqueues an AMBIENT-flagged task through the existing dispatch path (idempotent deterministic id, seed-card delivery, bot-sender loop prevention). Airtight per-channel default-OFF (global flag ∧ explicit channel allowlist). ALSO landed — brokered cross-channel flag (`@open-tag/cross-channel` pure-core `evaluateCrossChannelFlag` + `apps/api/cross-channel-tap.ts` `brokerCrossChannelFlag`): default-OFF two-layer gate (global `OPEN_TAG_CROSS_CHANNEL_ENABLED` ∧ per-(source,target) allowlist), fail-closed security exclusions checked BEFORE the allowlist (`self_target`, `private_source`, `private_target`, `cross_tenant`), every decision audited (`cross_channel.flag_decision`, no raw summary / no decline breadcrumbs), audit-fail-closed-on-delivery, delivery via `resolveChannelSender` with loop-prevention marker. Invocable-but-inert (no live raiser yet). See ADR-0006. ALSO landed — stale-thread nudge scanner (`@open-tag/ambient` pure-core `findStaleThreads` + `evaluateStaleThreadNudge`, IO edge `apps/api/stale-thread-scanner.ts` `scanStaleThreads`): notices `waiting_approval` tasks idle past a threshold (stale clock `max(task.updatedAt, session.updatedAt)`, `thread`/`waiting_delegation` excluded) and surfaces ONE gated/audited/idempotent nudge to the thread's OWN channel via `resolveChannelSender`. Default-OFF two-layer (`OPEN_TAG_STALE_THREAD_SCANNER_ENABLED` ∧ `OPEN_TAG_STALE_THREAD_CHANNELS`), idempotent via the audit log (`stale_thread.nudge`, one attempt per stale episode, no migration), same-channel private nudge allowed (not cross-channel leakage), audit carries no task goal/message text, loop-safe (direct delivery, never re-enters inbound; no row touched). Wired into the reconciler tick (primary-API-only, additive, error-isolated, re-entrancy-guarded); global flag off ⇒ complete no-op (no query). See ADR-0007. Pending: per-identity budget enforcement, richer proactive-framing judge, per-channel toggle moved into `chatConfigs`/UI, conversation-scoped sandbox, live flag-raising trigger + DB-backed cross-channel allowlist.
