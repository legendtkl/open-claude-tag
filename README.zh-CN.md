...
- **Channel** —— 聊天所在的平台。当前 Lark/飞书 是功能完整的 channel；Slack 是一个可用的
  第二 channel（入站分发 + 出站发送），已支持 OAuth 多 workspace 安装（ADR-0014）；Socket Mode
  仍在推进中。新增 channel 只需实现 `packages/channel-core` 里的 `Channel` 契约。
- **Runtime** —— 执行任务的引擎。Claude Code（默认）与 Codex 为完整 runtime。新增 runtime
  接入 `packages/runtime-adapters` 的 descriptor 驱动注册表即可。
...
| 变量 | 必填 | 默认 | 说明 |
| --- | --- | --- | --- |
| `SLACK_SIGNING_SECRET` | 启用 Slack 时 | — | 启用并签名校验 `POST /slack/events` 路由。也会处理 `app_uninstalled` / bot `tokens_revoked` 事件，禁用该 workspace 的安装。 |
...
| `SLACK_CLIENT_ID` | 启用 OAuth 安装时 | — | 与 `SLACK_CLIENT_SECRET` 一起，注册 `GET /slack/oauth/install` + `GET /slack/oauth/callback` 多 workspace 安装路由（ADR-0014）；未设则路由 404。 |
| `SLACK_CLIENT_SECRET` | 启用 OAuth 安装时 | — | 用于 `oauth.v2.access` 换取 token 的 Slack app client secret，需与 `SLACK_CLIENT_ID` 同时设置。 |
| `SLACK_OAUTH_REDIRECT_URI` | 否 | Slack app 默认 | OAuth 回调 URI，需与 Slack app 配置一致；省略则使用 app 配置的单一回调。 |
| `SLACK_STATE_SECRET` | 否 | 回退到 `SLACK_SIGNING_SECRET`，再到 `SLACK_CLIENT_SECRET` | 签名 OAuth `state`（CSRF + 归属）的 HMAC 密钥。 |
...
Slack 是一个可用的第二 channel，支持入站任务分发和出站 ACK，并**已支持 OAuth 多 workspace
安装**（ADR-0014）。其余仍**部分/实验性**：Socket Mode、更丰富的 running-card / Block Kit 对齐
尚未实现，也还没有用真实 workspace 凭据做过端到端验证。（worker 侧的终态完成回传是存在的 ——
见 ADR-0008。）

1. 设置 `SLACK_SIGNING_SECRET`。这会注册并签名校验 `POST /slack/events` 路由；不设则不挂载
   该路由。该路由还会处理 `app_uninstalled` / bot `tokens_revoked`，禁用对应 workspace 的安装
   （ADR-0014）。
...
3. 设置 `SLACK_BOT_TOKEN` 用于出站回传 —— 用于入站 ACK 的 Slack Web API token；worker 也会
   读取它来回传任务的终态完成（ADR-0008）。OAuth 安装会按 workspace 铸造并存储 token，所以这个
   env token 只是单 workspace 回退（ADR-0013）。

**OAuth 多 workspace 安装（ADR-0014）。** 设置 `SLACK_CLIENT_ID` + `SLACK_CLIENT_SECRET`
（以及可选的 `SLACK_OAUTH_REDIRECT_URI` / `SLACK_STATE_SECRET`）即可注册安装路由。已认证的控制台
用户打开 `GET /slack/oauth/install`，它会带着签名 `state` 302 跳转到 Slack 授权页；Slack 回跳到
`GET /slack/oauth/callback`，校验 state（CSRF + 发起方归属）、用 `oauth.v2.access` 换取 `code`，
并按 `team_id` upsert 该 workspace 的 per-team 安装（重装时保留原归属）。未设置 `SLACK_CLIENT_ID`
/ `SLACK_CLIENT_SECRET` 时这些路由 404，管理后台的手动 CRUD 路径（ADR-0013）不受影响。

把 Slack 应用的 Events API 请求 URL 指向 `https://<your-host>/slack/events`，并（启用 OAuth 时）
把 Redirect URL 指向 `https://<your-host>/slack/oauth/callback`。详见
[`doc/decisions/0005`](./doc/decisions/0005-neutral-non-lark-task-dispatch.md)、
[`0013`](./doc/decisions/0013-per-team-slack-token-model.md) 与
[`0014`](./doc/decisions/0014-slack-oauth-install-and-app-uninstall.md)。
